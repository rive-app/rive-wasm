...
template <typename D, typename S> SI D unchecked_bit_pun(const S& s)
{
    D d;
    memcpy(&d, &s, sizeof(D));
    return d;
...
template <typename D, typename S> SI Vec<1, D> cast(const Vec<1, S>& src) { return (D)src.val; }
